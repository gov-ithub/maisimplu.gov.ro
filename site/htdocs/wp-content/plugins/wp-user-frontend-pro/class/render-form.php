...
class WPUF_Render_Form {
    ...
    function render_items( $form_vars, $post_id, $type = 'post', $form_id, $form_settings, $cond_inputs = array() ) {
        ...
        foreach ($form_vars as $key => $form_field) {
            ...
            switch ($form_field['input_type']) {
                ...
                case 'file_upload':
                    $this->file_upload( $form_field, $post_id, $type, $form_id );
                    $this->conditional_logic( $form_field, $form_id );
                    break;
                ...
                case 'map':
                    $this->map( $form_field, $post_id, $type, $form_id );
                    $this->conditional_logic( $form_field, $form_id );
                    break;
     ...
     * @param array $attr
     */
    function toc( $attr, $post_id, $form_id ) {
    ...
    function date( $attr, $post_id, $type, $form_id ) {
        ...
        <div class="wpuf-fields">
            <input id="wpuf-date-<?php echo $attr['name']; ?>" type="text" class="datepicker <?php echo ' wpuf_'.$attr['name'].'_'.$form_id; ?>" data-required="<?php echo $attr['required'] ?>" data-type="text"<?php $this->required_html5( $attr ); ?> name="<?php echo esc_attr( $attr['name'] ); ?>" value="<?php echo esc_attr( $value ) ?>" size="30" />
            <span class="wpuf-help"><?php echo stripslashes( $attr['help'] ); ?></span>
        </div>
        <script type="text/javascript">
            jQuery(function($) {
     ...
     * @param int|null $post_id
     */
    function map( $attr, $post_id, $type, $form_id ) {
