jQuery(document).ready(function() {
    ...
    if ( (typeof zerif_nr_actions_required !== 'undefined') && (zerif_nr_actions_required != '0') ) {
        jQuery('li.mai-simplu-w-red-tab a').append('<span class="mai-simplu-actions-count">' + zerif_nr_actions_required + '</span>');
    }
    ...
    jQuery(".zerif-dismiss-required-action").click(function(){
        ...
        jQuery.ajax({
            ...
            beforeSend : function(data,settings){
				jQuery('.mai-simplu-tab-pane#actions_required h1').append('<div id="temp_load" style="text-align:center"><img src="' + zerifLiteWelcomeScreenObject.template_directory + '/inc/admin/welcome-screen/img/ajax-loader.gif" /></div>');
            },
            success    : function(data){
				jQuery("#temp_load").remove(); /* Remove loading gif */
                jQuery('#'+ data).parent().remove(); /* Remove required action box */

                var zerif_lite_actions_count = jQuery('.mai-simplu-actions-count').text(); /* Decrease or remove the counter for required actions */
                if( typeof zerif_lite_actions_count !== 'undefined' ) {
                    if( zerif_lite_actions_count == '1' ) {
                        jQuery('.mai-simplu-actions-count').remove();
                        jQuery('.mai-simplu-tab-pane#actions_required').append('<p>' + zerifLiteWelcomeScreenObject.no_required_actions_text + '</p>');
                    }
                    else {
                        jQuery('.mai-simplu-actions-count').text(parseInt(zerif_lite_actions_count) - 1);
                    }
                }
            },
	...
	function zerif_welcome_page_tabs(event) {
        ...
        jQuery(".mai-simplu-tab-pane").not(tab).css("display", "none");
        jQuery(tab).fadeIn();
	}
    ...
    jQuery(".mai-simplu-nav-tabs a").click(function(event) {
